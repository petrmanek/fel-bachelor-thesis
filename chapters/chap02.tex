...
Assuming that one hour of footage stored in the MultiFrame format may take up to 4~gigabytes in size (depending on the frequency of acquisition), we have 96~gigabytes per sensor per day. Accounting for the longest possible time of operation, our database will store up to 2,437~days of footage simultaneously recorded by up to 30~detectors. That means that our database will have to hold about 7~petabytes worth of uncompressed information. If we use Collin's compression algorithm benchmark as a baseline, we can estimate that a common variant of GZIP algorithm will reduce the file size in average by 75.9\%. Applying this compression on our MultiFrame data files, our database would have to hold \textit{only} about 1.7 petabytes of archives.



% CITACE: http://tukaani.org/lzma/benchmarks.html
